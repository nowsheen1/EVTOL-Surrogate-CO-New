...
function mBreakdownT = plotTradeResults_updated_wogb()
...
%     end
    if length(CTiltWing) >= i
        if ~isempty(massTiltWing_wo_gb(i).m)
            mT(i) = massTiltWing_wo_gb(i).m; 
        end
        if ~isempty(massTiltWing_wo_gb(i).battery)
            mBatT(i) = massTiltWing_wo_gb(i).battery; 
        end
        vT(i) = VTiltWing(i);
        ...
        mBreakdownT(i,:) = 1.1*[massTiltWing_wo_gb(i).payload, massTiltWing_wo_gb(i).avionics + ...
            massTiltWing_wo_gb(i).servos + massTiltWing_wo_gb(i).wire + massTiltWing_wo_gb(i).tilt, ...
            massTiltWing_wo_gb(i).seat + massTiltWing_wo_gb(i).brs, ...
            massTiltWing_wo_gb(i).battery, massTiltWing_wo_gb(i).motors ...
             massTiltWing_wo_gb(i).structural];
