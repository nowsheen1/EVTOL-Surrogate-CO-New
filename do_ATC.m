...
while atc.f_con>atc.tol
    ...
    for i=1:atc.bsize
        ...
        [atc.x_temp,atc.feval,atc.xflag]=fmincon(@fun,...
            ...
            atc.red_mat{i}*atc.lb,atc.red_mat{i}*atc.ub,atc.con...
            ,options,i,atc.v,atc.w,atc.cp_vec{i}...
            ,atc.var,atc.cp_mat,atc.red_mat,atc.vc,atc.wc,atc);
        ...
        atc.objc_pev(i)=obj(atc.x_temp,i,atc);
        
        % update v
        atc.v{i}=atc.v{i}+2*atc.w{i}.*atc.w{i}.*    (atc.diff);
        atc.vc(i)=atc.vc(i)+2*atc.wc(i)*atc.wc(i)*obj(atc.x_temp,i,atc);
        %diff=[];
        ...
        if atc.iter_main ~=1
            ...
            atc.wc(i)=atc.wc(i)*((obj(atc.x_temp,i,atc)>atc.gamma*atc.objc_pev(i))*atc.beta+...
                ~(obj(atc.x_temp,i,atc)>atc.gamma*atc.objc_pev(i)));
        else
        ...
        if i==1
            atc.fobjval=obj(atc.x_temp,i,atc);
            for j=1:atc.bsize
                if atc.constr(j)>0
                    atc.f_con=abs(obj(atc.x_temp,j,atc))+atc.f_con;
                end
            end
        end
    ...
    app.TextArea.Value=['   Design variables: ',...
        num2str(round(atc.x0(:,1)',1))];
    %     grid on
    %     set(ax,'XMinorTick','on','YMinorTick','on')
    drawnow
    
    %atc.beta=1+atc.cObjNew(atc.iter_main)*2;
...
save('mass_atc','mass_atc1');
app.TextArea.Value="ATC Complete.........."+newline+"Optimized variables : "+num2str(x_opt')+newline+"   Cost function value : "+num2str(res.fObjNew(res.iter_main));
